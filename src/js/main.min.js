window.addEventListener("load",()=>{let e=document.getElementById("preloader");e.style.opacity="0",e.style.transition="opacity 0.5s ease",setTimeout(()=>{e.style.display="none"},2e3)});
...
let scrollPositionContent=0;function disableScroll(){scrollPositionContent=window.pageYOffset||document.documentElement.scrollTop,document.body.classList.add("modal-open"),document.body.style.top=`-${scrollPositionContent}px`}function enableScroll(){document.body.classList.remove("modal-open"),document.body.style.top="",window.scrollTo(0,scrollPositionContent)}document.querySelectorAll(".open-modal").forEach(e=>{e.addEventListener("click",()=>{let t=e.getAttribute("data-target"),l=document.getElementById(t);l.classList.remove("hide"),l.classList.add("show"),l.style.display="block",disableScroll()})}),document.querySelectorAll(".modal .close").forEach(e=>{e.addEventListener("click",()=>{let t=e.closest(".modal");t.classList.remove("show"),t.classList.add("hide"),setTimeout(()=>{t.style.display="none",enableScroll()},400)})}),window.addEventListener("click",e=>{e.target.classList.contains("modal")&&(e.target.classList.remove("show"),e.target.classList.add("hide"),setTimeout(()=>{e.target.style.display="none",enableScroll()},400))}),document.querySelectorAll(".form-board").forEach(e=>{let t=e.querySelector(".radio-owner"),l=e.querySelector(".radio-manager"),o=e.querySelector(".employees-wrapper");t&&l&&o&&(t.addEventListener("change",()=>{t.checked&&(o.style.display="block")}),l.addEventListener("change",()=>{l.checked&&(o.style.display="none")}))}),document.querySelectorAll(".modal form").forEach(e=>{let t=e.querySelector('button[type="submit"]');e.addEventListener("submit",function(l){if(l.preventDefault(),e.checkValidity()){let o=document.documentElement.lang||"uk",s=t.getAttribute(`data-${o}-success`)||"Заявка відправлена ✓";t.textContent=s,t.style.color="white",e.reset();let r=e.querySelector("#employees-wrapper");r&&(r.style.display="none")}else e.reportValidity()})});
